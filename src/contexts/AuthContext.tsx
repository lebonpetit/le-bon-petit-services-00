...
export function AuthProvider({ children }: { children: React.ReactNode }) {
        ...
        try {
            ...
            if (error) {
                ...
                if (error.code === 'PGRST116') {
                    ...
                    // Tenants start with pending status (need payment approval), users are active immediately
    ...
    const signUp = async (email: string, password: string, name: string, phone: string, role: UserRole) => {
        ...
        const { data, error } = await supabase.auth.signUp({
            ...
            password,
            options: {
                emailRedirectTo: 'https://www.lebonpetit237.com/login',
                data: {
                    name,
                    phone,
                    role,
        ...
        if (data.user) {
            ...
            const { error: profileError } = await supabase.from('users').insert({
                ...
                name,
                role,
                status: role === 'tenant' ? 'pending' : 'active', // Tenants need payment validation, users are active
            });
    ...
    return (
        <AuthContext.Provider value={{ user, session, loading, signUp, signIn, signOut, updateProfile, refreshUser }}>
            {children}
