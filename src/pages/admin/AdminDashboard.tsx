...
import {
    ...
    Plus, Pencil, ShieldCheck, Loader, Play, UserCheck, Ban, FileText
} from 'lucide-react';
...
export default function AdminDashboard() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setStats({
                ...
                completedRequests: requestsData?.filter(r => r.status === 'completed' || r.status === 'processed').length || 0,
    ...
    const updateRequestStatus = async (
        ...
        try {
            ...
            setRequests(requests.map(r => r.id === requestId ? { ...r, ...updateData } as ServiceRequest : r));

            const statusMessages: Record<string, { title: string; description: string }> = {
                approved: { title: '✅ Demande approuvée', description: 'La demande a été acceptée et un agent a été affecté.' },
                rejected: { title: '❌ Demande refusée', description: 'La demande a été refusée.' },
                in_progress: { title: '▶️ Traitement démarré', description: 'La demande est maintenant en cours de traitement.' },
                completed: { title: '🎉 Demande traitée', description: 'La demande a été marquée comme traitée.' },
                processed: { title: '🎉 Demande traitée', description: 'La demande a été marquée comme traitée.' },
            };

            const msg = statusMessages[status] || { title: 'Statut mis à jour', description: `Statut changé en ${status}` };
            toast(msg);
        } catch (error) {
    ...
    const handleApproveRequest = async () => {
        if (!modalRequestId || !agentName.trim()) {
            toast({ variant: 'destructive', title: 'Erreur', description: 'Veuillez saisir le nom de l\'agent.' });
            return;
        }
        await updateRequestStatus(modalRequestId, 'approved', {
            assigned_agent_name: agentName.trim(),
    ...
    const handleRejectRequest = async () => {
        if (!modalRequestId || !rejectionReason.trim()) {
            toast({ variant: 'destructive', title: 'Erreur', description: 'Veuillez saisir le motif du refus.' });
            return;
        }
        await updateRequestStatus(modalRequestId, 'rejected', {
    ...
    const handleSaveProgressNote = async () => {
        ...
        await updateRequestStatus(modalRequestId, 'in_progress', {
            progress_note: progressNoteText.trim(),
        });
        setProgressNoteModalOpen(false);
        setProgressNoteText('');
        setModalRequestId(null);
    };
    ...
    const getStatusBadge = (status: string) => {
        const statusConfig: Record<string, { label: string; className: string }> = {
            new: { label: 'En attente', className: 'bg-blue-500 text-white' },
            approved: { label: 'Accepté', className: 'bg-emerald-500 text-white' },
            rejected: { label: 'Refusé', className: 'bg-red-500 text-white' },
            in_progress: { label: 'En cours', className: 'bg-amber-500 text-white' },
            completed: { label: 'Traité', className: 'bg-green-600 text-white' },
            processed: { label: 'Traité', className: 'bg-green-600 text-white' },
            cancelled: { label: 'Annulé', className: 'bg-gray-500 text-white' },
        };
                ...
                <DialogContent className="w-[95vw] max-w-lg max-h-[85vh] overflow-y-auto p-4 sm:p-6">
                            ...
                            <div className="grid gap-2 text-sm">
                                {request.assigned_agent_name && (
                                    <div className="flex justify-between">
                                        <span className="text-muted-foreground text-xs">Agent affecté</span>
                                ...
                                {request.approved_at && (
                                    <div className="flex justify-between">
                                        <span className="text-muted-foreground text-xs">Accepté le</span>
                                        <span className="font-medium text-xs">{new Date(request.approved_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                                    </div>
                                )}
                                {request.rejected_at && (
                                    <div className="flex justify-between">
                                        <span className="text-muted-foreground text-xs">Refusé le</span>
                                        <span className="font-medium text-xs text-red-500">{new Date(request.rejected_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                                    </div>
                                )}
                                {request.started_at && (
                                    <div className="flex justify-between">
                                        <span className="text-muted-foreground text-xs">Démarré le</span>
                                        <span className="font-medium text-xs">{new Date(request.started_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                                    </div>
                                )}
                                {request.completed_at && (
                                    <div className="flex justify-between">
                                        <span className="text-muted-foreground text-xs">Traité le</span>
                                        <span className="font-medium text-xs text-green-600">{new Date(request.completed_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                                    </div>
                                )}
                                {request.rejection_reason && (
                                    <div className="mt-2 p-2 bg-red-500/10 border border-red-500/20 rounded-lg">
                                        <span className="text-xs font-semibold text-red-600">Motif du refus :</span>
                                        <p className="text-sm mt-1">{request.rejection_reason}</p>
                                    </div>
                                )}
                                {request.progress_note && (
                                    <div className="mt-2 p-2 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                                        <span className="text-xs font-semibold text-amber-600">Note de progression :</span>
                                        <p className="text-sm mt-1">{request.progress_note}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                                ...
                                {Object.entries(payload).map(([key, value]) => {
                                    ...
                                    return (
                                        <div key={key} className="flex flex-col sm:flex-row sm:justify-between py-2 border-b border-border last:border-0 gap-1">
                                            ...
                                            {isLink ? (
                                                <a href={value as string} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline text-xs sm:text-sm font-medium truncate max-w-full sm:max-w-[60%]">Voir le logement →</a>
                                            ) : (
                                                <span className="font-medium text-xs sm:text-sm sm:text-right break-words max-w-full sm:max-w-[60%]">{formattedValue}</span>
                                            )}
                                        </div>
                                    );
                                })}
                    ...
                    <DialogFooter className="mt-4 flex-col sm:flex-row gap-2">
                        <a
                            ...
                            target="_blank" rel="noopener noreferrer" className="w-full sm:flex-1"
                        >
                        ...
                        {isAdminRequest && request.status === 'new' && (
                            <>
                                <Button variant="cta" className="w-full sm:flex-1" onClick={() => { setModalRequestId(request.id); setApproveModalOpen(true); }}>
                                    <CheckCircle className="h-4 w-4 mr-2" /> Approuver
                                </Button>
                                <Button variant="outline" className="w-full sm:flex-1 text-red-600 border-red-300 hover:bg-red-50" onClick={() => { setModalRequestId(request.id); setRejectModalOpen(true); }}>
                                    <Ban className="h-4 w-4 mr-2" /> Refuser
                                </Button>
                            </>
                        )}
                        {isAdminRequest && request.status === 'approved' && (
                            <Button variant="cta" className="w-full sm:flex-1" onClick={() => handleStartRequest(request.id)}>
                                <Play className="h-4 w-4 mr-2" /> Démarrer le traitement
                            </Button>
                        )}
                        {isAdminRequest && request.status === 'in_progress' && (
                            <Button variant="cta" className="w-full sm:flex-1 bg-green-600 hover:bg-green-700" onClick={() => handleCompleteRequest(request.id)}>
                                <CheckCircle className="h-4 w-4 mr-2" /> Marquer traité
                            </Button>
                        )}
                    </DialogFooter>
                </DialogContent>
        ...
        >
            ...
            <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-6">
                ...
                <TabsContent value="requests">
                    <Tabs defaultValue="pending" className="space-y-6">
                        <div className="bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 sticky top-14 z-10 w-full border-b pb-4">
                            <TabsList className="w-full justify-start h-auto p-1 bg-muted/50 flex-wrap">
                                <TabsTrigger value="pending" className="gap-1.5 data-[state=active]:bg-background data-[state=active]:shadow-sm text-xs sm:text-sm">
                                    <AlertTriangle className="h-4 w-4" />
                                    <span className="hidden sm:inline">À approuver</span>
                                    <span className="sm:hidden">Nouv.</span>
                                    {stats.newRequests > 0 && (
                                        <Badge variant="secondary" className="ml-1 h-5 min-w-[20px] rounded-full p-0 flex items-center justify-center bg-blue-500 text-white text-[10px]">
                                            {stats.newRequests}
                                        </Badge>
                                    )}
                                </TabsTrigger>
                                <TabsTrigger value="approved" className="gap-1.5 data-[state=active]:bg-background data-[state=active]:shadow-sm text-xs sm:text-sm">
                                    <CheckCircle className="h-4 w-4" />
                                    <span className="hidden sm:inline">Acceptées</span>
                                    <span className="sm:hidden">Acc.</span>
                                    {stats.approvedRequests > 0 && (
                                        <Badge variant="secondary" className="ml-1 h-5 min-w-[20px] rounded-full p-0 flex items-center justify-center bg-emerald-500 text-white text-[10px]">
                                            {stats.approvedRequests}
                                        </Badge>
                                    )}
                                </TabsTrigger>
                                <TabsTrigger value="in_progress" className="gap-1.5 data-[state=active]:bg-background data-[state=active]:shadow-sm text-xs sm:text-sm">
                                    <Loader className="h-4 w-4" />
                                    <span className="hidden sm:inline">En cours</span>
                                    <span className="sm:hidden">Cours</span>
                                    {stats.inProgressRequests > 0 && (
                                        <Badge variant="secondary" className="ml-1 h-5 min-w-[20px] rounded-full p-0 flex items-center justify-center bg-amber-500 text-white text-[10px]">
                                            {stats.inProgressRequests}
                                        </Badge>
                                    )}
                                </TabsTrigger>
                                <TabsTrigger value="completed" className="gap-1.5 data-[state=active]:bg-background data-[state=active]:shadow-sm text-xs sm:text-sm">
                                    <CheckCircle className="h-4 w-4" />
                                    <span className="hidden sm:inline">Traitées</span>
                                    <span className="sm:hidden">Fait</span>
                                </TabsTrigger>
                                <TabsTrigger value="rejected" className="gap-1.5 data-[state=active]:bg-background data-[state=active]:shadow-sm text-xs sm:text-sm">
                                    <XCircle className="h-4 w-4" />
                                    <span className="hidden sm:inline">Refusées</span>
                                    <span className="sm:hidden">Ref.</span>
                                </TabsTrigger>
                                <TabsTrigger value="history" className="gap-1.5 data-[state=active]:bg-background data-[state=active]:shadow-sm text-xs sm:text-sm">
                                    <Clock className="h-4 w-4" />
                        ...
                        {(() => {
                            const renderRequestTable = (filterFn: (r: ServiceRequest) => boolean, emptyMessage: string, showActions: boolean = true) => (
                                <Card>
                                    <CardContent className="pt-6">
                                        <div className="overflow-x-auto">
                                            <Table>
                                                ...
                                                <TableBody>
                                                    {filteredRequests.filter(filterFn).map((request) => (
                                                        <TableRow key={request.id}>
                                                            ...
                                                            <TableCell className="font-medium">
                                                                <div className="flex flex-col">
                                                                    ...
                                                                    <span className="text-xs text-muted-foreground">{request.contact_phone}</span>
                                                                </div>
                                                            </TableCell>
                                                            <TableCell className="hidden md:table-cell">
                                                                {request.assigned_agent_name ? (
                                                            ...
                                                            {showActions && (
                                                                <TableCell>
                                                                    <div className="flex gap-1.5 flex-wrap">
                                                                        ...
                                                                        {request.status === 'new' && (
                                                                            <>
                                                                                <Button size="sm" variant="outline" className="text-emerald-600 border-emerald-300 hover:bg-emerald-50 text-xs" onClick={() => { setModalRequestId(request.id); setApproveModalOpen(true); }}>
                                                                                    <CheckCircle className="h-3.5 w-3.5 mr-1" /> Approuver
                                                                                </Button>
                                                                                <Button size="sm" variant="outline" className="text-red-600 border-red-300 hover:bg-red-50 text-xs" onClick={() => { setModalRequestId(request.id); setRejectModalOpen(true); }}>
                                                                                    <Ban className="h-3.5 w-3.5 mr-1" /> Refuser
                                                                                </Button>
                                                                            </>
                                                                        )}
                                                                        {request.status === 'approved' && (
                                                                            <Button size="sm" variant="outline" className="text-amber-600 border-amber-300 hover:bg-amber-50 text-xs" onClick={() => handleStartRequest(request.id)}>
                                                                                <Play className="h-3.5 w-3.5 mr-1" /> Démarrer
                                                                            </Button>
                                                                        )}
                                                                        {request.status === 'in_progress' && (
                                                                            <>
                                                                                <Button size="sm" variant="outline" className="text-green-600 border-green-300 hover:bg-green-50 text-xs" onClick={() => handleCompleteRequest(request.id)}>
                                                                                    <CheckCircle className="h-3.5 w-3.5 mr-1" /> Traité
                                                                                </Button>
                                                                                <Button size="sm" variant="outline" className="text-xs" onClick={() => { setModalRequestId(request.id); setProgressNoteText(request.progress_note || ''); setProgressNoteModalOpen(true); }}>
                                                                                    <FileText className="h-3.5 w-3.5 mr-1" /> Note
                                                                                </Button>
                                                                            </>
                                                                        )}
                                                                        {!request.landlord_id && (request.status === 'new' || request.status === 'cancelled') && (
                                                                            <AlertDialog>
                                                                                <AlertDialogTrigger asChild>
                                                    ...
                                                    {filteredRequests.filter(filterFn).length === 0 && (
                                                        <TableRow>
                                    ...
                                    <TabsContent value="pending">
                                        <CardHeader className="px-0 pt-0">
                                            <CardTitle className="flex items-center gap-2 text-lg">
                                                ...
                                                Demandes à approuver ({filteredRequests.filter(r => r.status === 'new').length})
                                            </CardTitle>
                                        </CardHeader>
                                        {renderRequestTable(r => r.status === 'new', 'Aucune demande à approuver')}
                                    </TabsContent>
                                    ...
                                    <TabsContent value="approved">
                                        <CardHeader className="px-0 pt-0">
                                            <CardTitle className="flex items-center gap-2 text-lg">
                                                ...
                                                Demandes acceptées ({filteredRequests.filter(r => r.status === 'approved').length})
                                            </CardTitle>
                                        </CardHeader>
                                        {renderRequestTable(r => r.status === 'approved', 'Aucune demande acceptée')}
                                    </TabsContent>
                                    ...
                                    <TabsContent value="in_progress">
                                        <CardHeader className="px-0 pt-0">
                                            <CardTitle className="flex items-center gap-2 text-lg">
                                                ...
                                                Demandes en cours ({filteredRequests.filter(r => r.status === 'in_progress').length})
                                            </CardTitle>
                                        </CardHeader>
                                        {renderRequestTable(r => r.status === 'in_progress', 'Aucune demande en cours')}
                                    </TabsContent>
                                    ...
                                    <TabsContent value="completed">
                                        <CardHeader className="px-0 pt-0">
                                            <CardTitle className="flex items-center gap-2 text-lg">
                                                ...
                                                Demandes traitées ({filteredRequests.filter(r => r.status === 'completed' || r.status === 'processed').length})
                                            </CardTitle>
                                        </CardHeader>
                                        {renderRequestTable(r => r.status === 'completed' || r.status === 'processed', 'Aucune demande traitée')}
                                    </TabsContent>
                                    ...
                                    <TabsContent value="rejected">
                                        <CardHeader className="px-0 pt-0">
                                            <CardTitle className="flex items-center gap-2 text-lg">
                                                ...
                                                Demandes refusées ({filteredRequests.filter(r => r.status === 'rejected').length})
                                            </CardTitle>
                                        </CardHeader>
                                        {renderRequestTable(r => r.status === 'rejected', 'Aucune demande refusée')}
                                    </TabsContent>
                                    ...
                                    <TabsContent value="history">
                                        <CardHeader className="px-0 pt-0">
                                            <CardTitle className="flex items-center gap-2 text-lg">
                                                ...
                                                Historique complet ({filteredRequests.length})
                                            </CardTitle>
                                        </CardHeader>
                                        {renderRequestTable(() => true, 'Aucune demande')}
                                    </TabsContent>
                                </>
                            );
                        })()}
                    ...
                    <Dialog open={approveModalOpen} onOpenChange={setApproveModalOpen}>
                        <DialogContent className="w-[95vw] max-w-md">
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    Saisissez le nom de l'agent qui sera affecté à cette tâche.
                                </DialogDescription>
                            </DialogHeader>
                            ...
                            <DialogFooter className="gap-2">
                                <Button variant="outline" onClick={() => { setApproveModalOpen(false); setAgentName(''); }}>
                                    Annuler
                                </Button>
                                <Button variant="cta" onClick={handleApproveRequest} disabled={!agentName.trim()}>
                                    <CheckCircle className="h-4 w-4 mr-2" />
                                    Approuver et affecter
                                </Button>
                            </DialogFooter>
                        </DialogContent>
                    </Dialog>
                    ...
                    <Dialog open={rejectModalOpen} onOpenChange={setRejectModalOpen}>
                        <DialogContent className="w-[95vw] max-w-md">
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    Indiquez le motif du refus. Le client sera informé.
                                </DialogDescription>
                            </DialogHeader>
                            <div className="space-y-4 py-4">
                                <div className="space-y-2">
                                    ...
                                    <Textarea
                                        ...
                                        placeholder="Ex: Service non disponible dans cette zone..."
                    ...
                    {/* Progress Note Modal */}
                    <Dialog open={progressNoteModalOpen} onOpenChange={setProgressNoteModalOpen}>
                        <DialogContent className="w-[95vw] max-w-md">
                            <DialogHeader>
                                <DialogTitle className="flex items-center gap-2">
                                ...
                                <DialogDescription>
                                    Ajoutez ou modifiez la note de progression visible par le client.
                                </DialogDescription>
                            </DialogHeader>
                            <div className="space-y-4 py-4">
                                <div className="space-y-2">
                                    ...
                                    <Textarea
                                        ...
                                        placeholder="Ex: Préparation en cours, livraison prévue demain..."
                                        className="min-h-[100px]"
                                        autoFocus
                                    />
                                </div>
                            ...
                            <DialogFooter className="gap-2">
                                <Button variant="outline" onClick={() => { setProgressNoteModalOpen(false); setProgressNoteText(''); }}>
                                    Annuler
                                </Button>
                                <Button variant="cta" onClick={handleSaveProgressNote}>
