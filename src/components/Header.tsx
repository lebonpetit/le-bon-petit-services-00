...
import { Menu, X, Home, Package, Flame, Shirt, Trash2, Building2, User, LogOut, Sparkles, Truck, ChevronDown, Users, Star } from "lucide-react";
                  ...
                  {user.role === 'user' && (
                    <DropdownMenuItem asChild>
                      <Link to="/user/dashboard" className="cursor-pointer">
                        <Star className="h-4 w-4 mr-2 text-african-yellow" />
                        {user.loyalty_points || 0} pts fidélité
                        {user.discount_available && <span className="ml-1 text-green-500 text-xs">(-30%!)</span>}
                      </Link>
                    </DropdownMenuItem>
                  )}
                  <DropdownMenuSeparator />
